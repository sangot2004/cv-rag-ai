...
def has_already_sent(candidate_id: str) -> bool:
    with SessionLocal() as session:
        existing = (
            session.query(SentEmail.id).filter(SentEmail.candidate_id == candidate_id).first()
        )
        return existing is not None


def send_interview_invitation(candidate_id: str, subject: str, body: str) -> dict:
    with SessionLocal() as session:
    ...
    try:
        ...
        logger.exception("send_interview_invitation lỗi khi gửi qua Gmail")
